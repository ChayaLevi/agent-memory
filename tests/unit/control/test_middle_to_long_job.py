...
def test_split_by_continuity_continuous_within_batch_size() -> None:
    ...
    job._llm._responses = [
        '{"results":["true"]}'
    ] * 5  # 6 个 unit → 5 次比较  # pylint: disable=protected-access
...
def test_split_by_continuity_breaks_on_false() -> None:
    ...
    job._llm._responses = [
        '{"results":["false"]}'
    ] * 3  # 4 个 unit → 3 次比较全 false  # pylint: disable=protected-access
...
def test_split_by_continuity_respects_batch_size_upper_bound() -> None:
    ...
    job._llm._responses = [
        '{"results":["true"]}'
    ] * 5  # 全部连续  # pylint: disable=protected-access
    units = [_make_unit(f"u{i}", scope, f"c{i}") for i in range(6)]
...
def test_run_serial_path_calls_evolver_and_archives_processed() -> None:
    ...
    job._llm._responses = [
        '{"results":["true"]}'
    ]  # 2 unit → 1 次比较  # pylint: disable=protected-access
