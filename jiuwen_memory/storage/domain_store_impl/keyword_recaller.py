...
"""最小实现：:class:`~storage.domain_store_impl.recaller.Recaller` 的 KEYWORD 通道。
...
``CompositeDomainStore.recall`` 负责，本通道不感知其他通道。
...
class KeywordRecaller(Recaller):
    ...
    def __init__(
        ...
        *,
        ...
        # 端口 + load_units（运行期持最小接口，F07 决策 10 修订）。kv 端口是
...
def _build(config):
    ...
    # manager 的同一端口，读写共享同一实例由 manager 保证（F07），不再依赖两侧各自
